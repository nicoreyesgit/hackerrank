...
func TestFindInRotateArray(t *testing.T) {
	t.Run("should return then", func(t *testing.T) {
		...
		r := FindInRotateArrayRecursive(arr, 0, 8, 15)
		...
		r = FindInRotateArrayRecursive(arr2, 0, 7, 1)
		if r != 7 {
			t.Error("the index should be 7 and is ", r)
		}

		arr3 := []int{6, 8, 9, 10, 1, 2, 3, 1}
		r = FindInRotateArrayRecursive(arr3, 0, 7, 2)
		if r != 5 {
			t.Error("the index should be 7 and is ", r)
		}

		arr4 := []int{1, 2, 3, 5, 7, 8, 10, 11}
		r = FindInRotateArrayRecursive(arr4, 0, 7, 7)
		if r != 4 {
			t.Error("the index should be 7 and is ", r)
		}

		arr5 := []int{5, 6, 7, 8, 9, 10, 1, 2, 3}
		r = FindInRotateArrayRecursive(arr5, 0, 8, 9)
		if r != 3 {
			t.Error("the index should be 4 and is ", r)
		}
	})

	t.Run("should return -1", func(t *testing.T) {
		arr := []int{6, 8, 9, 10, 11, 12, 13, 1}
		r := FindInRotateArrayRecursive(arr, 0, 7, 14)
		if r != -1 {
